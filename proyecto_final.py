#Libreria encargada de plotear los diagramas de radiación
import matplotlib.pyplot as plt
#Librerias para hacer uso de funciones matemáticas e integrales
import scipy
from scipy.integrate import quad
import numpy as np

#Funcion para calcular la Longitud practica del dipolo dependiendo de su longitud
...
#Parte 1 de la función del campo E para la integral de potencia radiada
def E(x,Im,r,k,H):
    n = 120*np.pi
    return ((n*Im/(2*np.pi*r))*((np.cos(k*H*np.cos(x))-np.cos(k*H))/np.sin(x)))**2*r**2*np.sin(x)/n  #x = tetha

#Parte 2 de la integral de potencia radiada
def I2(x):
    return 1

#Función del cálculo de la potencia radiada
def potencia_radiada(Im,r,f,L):                      #L=2H
    ...
    if(P2*P1 < 1):                                   #por si la potencia es < 1, se expresa en mW
        print("\t\tEn miliwatts")
        print("\t\t{0:.5f}".format(P2*P1*1000),"mW") #se restringe a una presición de 5 decimales
    return P1*P2

#Cálculo de la resistencia de radiación
def R_r(P,Im):
    Rr = 2*P/Im
    print("\t\t{0:.5f}".format(Rr),"ohms")           #se restringe a una presición de 5 decimales
    return Rr

#Cálculo de la resistencia de pérdidas
def R_p(f,L, a, o):	                             #L: longitud de la antena, a: radio de la antena, o: conductancia del material
    ...
    Rp = Rs*L/(2*np.pi*a)
    print("\t\t{0:.5f}".format(Rp), "ohms")          #se restringe a una presición de 5 decimales
    return Rp

#Cálculo de la reactancia
def reactancia(L,a,f):
    #Se asume la reactancia igual a cero (antena en resonancia) ya que k*H da multiplos de pi/2 y da infinito
    #H = L/2
    #Zo = 120*(np.log(2*H/a)-1)
    #k = 2*np.pi/(3*10**8/f)                         #k = numero de onda
    #X = -Zo*(1/np.tan(H*k))
    X = 0
    print("\t\t{0:.1f}".format(X),"j ohms")          #se restringe a una presición de 5 decimales
    return X

#Calculo de la eficiencia
def e_cd(Rr, Rp):
    ecd = Rr/(Rr+Rp)
    print("\t\t{0:.5f}".format(ecd))                 #se restringe a una presición de 5 decimales
    return ecd

#Cálculo del máximo del campo eléctrico
def max_E(n,Im,r,k,H):
    maxim = 0.0
    aux = 0.0
    #como python no deja incrementar de a 0.01 el for, se incrementa de a 1 y la x se divide en 100 en los calculos,
    #2pi=6.3 y se multiplica por 100 para la equivalencia
...
#Parte 1 de la función del campo E normalizado para la integral del ancho del haz
def En(x,Im,r,k,H):
    n = 120*np.pi
...
def directividad(ancho):
    D = 4*np.pi/ancho
    print("\t\t{0:.5f}".format(D))                   #Se restringe a una presición de 5 decimales
    return D

#Cálculo de la apertura efectiva
def apertura_Efec(D, lambd):
    Ap_Efec = (D*lambd**2)/(4*np.pi)
    print("\t\t{0:.5f}".format(Ap_Efec))             #Se restringe a una presición de 5 decimales
    return Ap_Efec

#Cálculo de la ganancia de la antena
def gain_antena(e_cd, D):
    G = e_cd*D
    print("\t\t{0:.5f}".format(G),"veces")           #Se restringe a una presición de 5 decimales
    #Se pasa a db's
    print("\t\t{0:.5f}".format(10*np.log10(G)),"dBi")#Se restringe a una presición de 5 decimales
    return G

#Función donde se hace la interfaz gráfica de la sección del dipolo
def dipolo():
    print("\n\n════════════════════════════════════════════════════════════════════════════")
    print("Este trabajo calcula los parámetros de la antena dipolo de λ/2, λ, 3λ/2 o 2λ")
    print("dados los siguientes parámetros:\n")
    print("════════════════════════════════════════════════════════════════════════════")
    print("\n>>>Ingrese la frecuencia de trabajo [Hz]:")
    fo = float(input())
    #Se verifica que no ingrese terminos negativos
    while(fo <= 0):
        print("\n═════════════════════════════════")
    ...
    while ((L <= 1/10) or (d == 0) or ((n/d != 1/2) and (n/d != 3/2) and (n/d != 1) and (n/d != 2))):
            ...
            if (d != 0):
                L = n/d
    #Se cálcula la longitud teórica de la antena
    L =(3*10**8/fo)*L
    ra = 1
    ...
    while(o < 0):
        ...
        o = float(input())
    #Cálculo y escritura de los parámetros
    print("\n\n═════════════════════════════════════")
    ...
    print("═════════════════════════════════════")
    print("\n\t>>>Lambda (λ):")
    print("\t\t{0:.5f}".format(3*10**8/fo), "m")          #Se restringe a una presición de 5 decimales
    print("\n\t>>>L nominal:")
    print("\t\t",L, "m")
    print("\n\t>>>L práctica:")
    print("\t\t{0:.5f}".format(L_prac(L,di,n,d,fo)), "m") #Se restringe a una presición de 5 decimales
    print("\n\t>>>Potencia Radiada")
    ...
    X = reactancia(L,a,fo)
    print("\n\t>>>Impedancia")
    #Corrección de forma de escritura si es negativo o positivo la parte compleja
    if X >= 0: 
    ...
    H=L/2
    #Graficación del diagrama de radiación en coordenadas polares
...
#Función donde se hace la interfaz gráfica de la sección de la guía de onda
def guia_onda():
    print("\n\n═════════════════════════════════════════════════════════════════════════════")
    ...
    print("\t\t{0:.5f}".format(fc1/1000000000), "GHz            Z<0")     #Se restringe a una presición de 5 decimales
    fc2 = ((3*10**8)/2)*np.sqrt((m/a)**2+(n/b)**2)
    print("\t\t{0:.5f}".format(fc2/1000000000), "GHz            Z>0")     #Se restringe a una presición de 5 decimales
    #Cálculo de longitud de onda de corte
    print("\t>>>Longitud de onda de corte")
    lam1 = 3*10**8/fc1
    lam2 = 3*10**8/fc2
    print("\t\t{0:.5f}".format(lam1), "m              Z<0")               #Se restringe a una presición de 5 decimales
    print("\t\t{0:.5f}".format(lam2), "m              Z>0")               #Se restringe a una presición de 5 decimales
    #Cálculo de Impedancia modo TE10
    ...
    print("\t\t{0:.5f}".format(z1), "ohms         Z<0")                   #Se restringe a una presición de 5 decimales
    print("\t\t{0:.5f}".format(z2), "ohms         Z>0")                   #Se restringe a una presición de 5 decimales
    #Cálculo de velocidad de fase
    ...
    print("\t\t{0:.5f}".format(vfas1), "m/s            Z<0")              #Se restringe a una presición de 5 decimales
    print("\t\t{0:.5f}".format(vfas2), "m/s            Z>0")              #Se restringe a una presición de 5 decimales
    #Cálculo de velocidad de grupo
    ...
    print("\t\t{0:.5f}".format(vgrup/1000), "Km/s      Z<0")              #Se restringe a una presición de 5 decimales

#Función donde se hace la interfaz gráfica de la sección del circuito de microonda
def circuit_micro():
    print("\n\n═════════════════════════════════════════════════════════════════════════════")
    ...
    #Definición de matriz de magnitud y fase
    ...
    sumat = 0
    #Sumatoria de la primera fila de la matriz de magnitud
    for i in range(0,4):
    ...
    cont = 0
    #Verificación de matriz simétrica
    for i in range(0,4):
        for j in range(0,4):
...
#Descripción del menú de entrada principal
def main():
    flag = 0
    print("\n\n═════════════════════════════════════════════════════════════════════════════")
    ...
    while flag == 0:
        ...
        print("\t1)Parámetros de antena de longitud n lambda.")
        print("\t2)Parámetros de guía de onda.")
        print("\t3)Circuitos de microondas.\n")
        flag = int(input("Ingrese su opción: "))
        #Se verifica de que solo pueda ingresar las opciones dadas
